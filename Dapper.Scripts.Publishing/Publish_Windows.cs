...
namespace Dapper.Scripts.Publishing
{
    public class Publish_Windows : IBuildTask
    {
        ...
        {
            var assemblyFilename = Path.Combine(Context.ContentDirectory, "Dapper.Scripts", "bin", "Release", "Dapper.Scripts.dll");
            ...
            var nugetTool = new NuGetPackagePublisher(Context) {
                ...
                PackageId = "dapper.scripts",
                PackageDefinition = Path.Combine(Context.ContentDirectory, "Dapper.Scripts", "Dapper.Scripts.csproj"),
